    ...
    <>
      ...
      <main>
        <section className="section-padding border-y border-border bg-off-white"><div className="container-main grid grid-cols-1 gap-6 lg:grid-cols-[.72fr_1.28fr]">
          <aside className="border-t-4 border-saffron bg-white p-5"><h2 className="text-xl font-bold">Location profile</h2><dl className="mt-4 space-y-3 text-sm"><div><dt className="text-muted-fg">Capital</dt><dd className="font-bold">{location.capital}</dd></div><div><dt className="text-muted-fg">Region</dt><dd className="font-bold">{location.region}</dd></div><div><dt className="text-muted-fg">Opportunity count</dt><dd className="font-bold">{items.length}</dd></div></dl></aside>
        ...
        <section className="section-padding"><div className="container-main"><p className="text-xs font-bold uppercase tracking-wider text-royal">Investment Opportunities</p><h2 className="mt-2 text-3xl font-bold">Opportunities in {location.name}</h2>{items.length ? <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">{items.map((item) => <OpportunityCard key={item.id} opportunity={item} />)}</div> : <div className="mt-6 border border-border bg-off-white p-6"><h3 className="text-lg font-bold">No opportunities have been added for this location yet.</h3><p className="mt-2 text-sm text-muted-fg">The location remains part of the national directory. Return to the map or explore all opportunities.</p></div>}<div className="mt-8 flex flex-wrap gap-3"><Link to="/bharat-investment-grid#india-map" className="inline-flex min-h-11 items-center gap-2 border border-navy-deep px-4 py-2 font-bold text-navy-deep"><ArrowLeft size={16} /> Back to India map</Link><Link to="/bharat-investment-grid#opportunities" className="inline-flex min-h-11 items-center bg-navy-deep px-4 py-2 font-bold text-white">Explore all opportunities</Link></div></div></section>
      </main>
    </>
  )
}
