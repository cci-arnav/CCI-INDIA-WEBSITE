  ...
  return (
    <div className="big-page">
      <section className="relative overflow-hidden bg-navy-deep text-white">
        ...
        <div className="container-main grid grid-cols-1 gap-9 py-12 md:py-16 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
          <div>
          ...
          <div className="border border-white/20 bg-white/5 p-5 sm:p-6">
            <label htmlFor="big-hero-search" className="text-sm font-bold text-white">Find an illustrative opportunity</label>
        ...
        <div className="container-main">
          ...
          {filteredLocations.length ? <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{filteredLocations.map((item) => (
            <article key={item.slug} className="flex h-full flex-col border border-border bg-white p-5">
              ...
              <p className="mt-3 text-sm font-bold text-royal">{getOpportunityCount(item.slug)} Illustrative {getOpportunityCount(item.slug) === 1 ? 'Opportunity' : 'Opportunities'}</p>
      ...
      <section ref={opportunityRef} id="opportunities" className="section-padding scroll-mt-32 border-b border-border bg-off-white">
        <div className="container-main">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4"><h2 className="mt-2 text-3xl font-bold">Illustrative Opportunities</h2><button type="button" onClick={resetOpportunities} className="min-h-11 border border-navy-deep px-4 text-sm font-bold text-navy-deep">Reset all filters</button></div>
          ...
          <p className="mt-4 text-sm text-muted-fg" role="status">{filteredOpportunities.length} illustrative {filteredOpportunities.length === 1 ? 'opportunity' : 'opportunities'} found.</p>
      ...
      <section className="section-padding bg-navy-deep text-white">
        <div className="container-main">
          <p className="text-xs font-bold uppercase tracking-wider text-saffron">Illustrative Prototype Data</p><h2 className="mt-2 text-3xl font-bold text-white">India Investment Landscape</h2>
          <div className="mt-7 grid grid-cols-2 gap-px bg-white/15 md:grid-cols-4">{[[28, 'States'], [8, 'Union Territories'], [sectors.length, 'Prototype sectors'], [opportunities.length, 'Illustrative opportunities']].map(([value, label]) => <div key={label} className="bg-navy-deep p-5"><strong className="block font-serif text-3xl text-saffron">{value}</strong><span className="text-sm text-white/75">{label}</span></div>)}</div>
        </div>
      </section>
      ...
      <section className="section-padding border-b border-border">
        <div className="container-main"><h2 className="text-3xl font-bold">Why Bharat Investment Grid?</h2><div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">{[
          [Compass, 'DISCOVER', 'Find opportunities across India’s diverse regions and sectors.'], [GitCompareArrows, 'COMPARE', 'Explore illustrative opportunities across States and Union Territories.'], [Users, 'CONNECT', 'Review opportunities and express interest through one interface.'], [BarChart3, 'EXPLORE', 'Understand India’s regional investment landscape through a unified platform.'],
