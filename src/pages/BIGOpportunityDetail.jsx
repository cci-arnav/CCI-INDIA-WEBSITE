...
export default function BIGOpportunityDetail() {
  ...
  if (!item) return (
    <section className="section-padding"><div className="container-main max-w-3xl"><p className="text-xs font-bold uppercase tracking-wider text-saffron">Opportunity not found</p><h1 className="mt-2 text-3xl font-bold">This illustrative opportunity is unavailable</h1><p className="mt-3 text-muted-fg">The link may be incomplete or the prototype record may have changed.</p><Link to="/bharat-investment-grid#opportunities" className="mt-6 inline-flex min-h-11 items-center gap-2 bg-navy-deep px-5 py-2 font-bold text-white"><ArrowLeft size={16} /> Browse opportunities</Link></div></section>
  )

  const location = getLocation(item.stateSlug)
  return (
    <>
      <header className="bg-navy-deep text-white"><div className="container-main py-10 md:py-14"><nav aria-label="Breadcrumb" className="text-xs text-white/65"><Link to="/">Home</Link> / <Link to="/bharat-investment-grid">Bharat Investment Grid</Link> / <span aria-current="page">Opportunity detail</span></nav><p className="mt-7 text-xs font-bold uppercase tracking-[0.18em] text-saffron">Illustrative Opportunity</p><h1 className="mt-2 max-w-4xl text-3xl font-bold text-white sm:text-4xl">{item.name}</h1><p className="mt-4 flex items-center gap-2 text-sm text-white/70"><MapPin size={16} aria-hidden="true" /> {location?.name} · {location?.region}</p></div></header>
