...
3. Copy the project URL and anon/publishable key into `.env` as `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`. Never add a service-role key to this frontend.
...
For Vercel, add the same two `VITE_` environment variables to the project and redeploy. `vercel.json` already rewrites direct route visits to the SPA entry point.
